...
| **`crates/h264-decoder`** | 20 unit tests on the foundational bottom-up modules: `bitreader` (raw bit reads MSB-first across byte boundaries; `read_ue`/`read_se`/`read_te` against H.264 spec Table 9-1 vectors; pathological-long-zeros rejection; truncation propagation) and `nal` (3-byte + 4-byte Annex-B start codes; trailing-zero padding strip; `nal_unit_type`/`nal_ref_idc` extraction with `forbidden_zero_bit` rejection; emulation-prevention byte removal in all positions; **live integration test that parses the x264 corpus and confirms SPS → PPS → IDR ordering**). Crate is no_std-pure. | PASS | `cargo test -p snarkvid-h264-decoder` |
