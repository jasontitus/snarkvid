...
| **`crates/h264-decoder`** | 95 unit tests across seven bottom-up modules. Highlights: live x264 corpus parse confirming SPS → PPS → IDR ordering; coeff_token decoder against H.264 Table 9-5 known codewords; 4×4 IDCT 1D butterfly verified against spec §8.5.12; FDCT → quant → dequant → IDCT round-trip recovers the original block to within ±1; slice.rs parses the live x264 corpus (16×16 dims, IDR slice, SliceQPY=15); **all 17 intra prediction modes** (9 Intra_4×4 + 4 Intra_16×16 + 4 chroma 8×8) including Plane mode horizontal-gradient validation; **end-to-end `decode_residual_block_4x4` composition pulls coeff_token + trailing-ones signs + level prefix/suffix + total_zeros + run_before + zig-zag swizzle into 16 raster-order coefficients** (a hand-built bitstream with TC=1, T1=1, sign=+1, total_zeros=15 produces a block with exactly +1 at the DC raster position). Crate is no_std-pure. | PASS | `cargo test -p snarkvid-h264-decoder` |
...
| **M3 H.264 decoder modules still TODO** | `bitreader`, `nal`, `cavlc` (coeff_token + level + total_zeros TC≤2 + run_before zeros_left≤3 + decode_residual_block composition), `transform`, `quant`, `slice`, `intra` (all 17 modes) are committed. **Still TODO**: total_zeros tables for TC=3..15 (~250 entries); run_before tables for zeros_left=4..14 (~80 entries); VLC0 coeff_token rows TC=9..16 (~30 entries); full VLC1 (nC ∈ [2,4)) and VLC2 (nC ∈ [4,8)) coeff_token tables (~120 entries); VUI block parser inside SPS (currently skipped — works for our corpus); Hadamard-DC quant variants (Intra_16×16 luma DC and chroma DC); `mb` (per-MB decode loop), `frame` (top-level decode_iframe), validation against ffmpeg-decoded reference YUV. The remaining tables are mechanical transcription from spec Tables 9-7 / 9-9 / 9-10. | Differential fuzz against ffmpeg/JM (M3 §3) lands once `frame::decode_iframe` is functional; runs on CPU but takes wall-clock budget that suits a GPU box. |
