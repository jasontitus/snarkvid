...
| **M2 native pipeline** (`bin/toy-encode/tests/m2_pipeline.rs`) | 7 integration tests composing manifest + toy-codec + comparator end-to-end: qp=0 happy-path at 60 dB, qp=8 happy-path at 36 dB, plus the three §3.4 tampering modes that don't need a prover (compressed bit-flip → PSNR fails; manifest signed by unknown key → sig fails; tolerance below actual PSNR → fails); plus corrupted-Merkle-path and path-count-mismatch sanity | PASS | `cargo test -p toy-encode --test m2_pipeline` |
