...
| **`crates/h264-decoder`** | 28 unit tests on the foundational bottom-up modules: `bitreader` (raw bit reads MSB-first across byte boundaries; `read_ue`/`read_se`/`read_te` against H.264 spec Table 9-1 vectors; pathological-long-zeros rejection; truncation propagation), `nal` (3-byte + 4-byte Annex-B start codes; trailing-zero padding strip; `nal_unit_type`/`nal_ref_idc` extraction with `forbidden_zero_bit` rejection; emulation-prevention byte removal in all positions; **live integration test that parses the x264 corpus and confirms SPS → PPS → IDR ordering**), and `cavlc` (the `read_vlc` table primitive incl. failed-lookup non-advance; the four `coeff_token` variants per Table 9-5: VLC0 nC<2 known codewords TC=0..7, chroma DC 4:2:0 known codewords, fixed-length-6 nC≥8 with the special-case 0b000011 sentinel, variant selector from nC). Crate is no_std-pure. | PASS | `cargo test -p snarkvid-h264-decoder` |
...
| **M3 H.264 decoder modules above the bottom layer** | `bitreader` and `nal` are committed and tested. The next M3 modules (`cavlc`, `slice`, `transform`, `quant`, `intra`, `mb`, `frame`) are not yet implemented. Each is bottom-up testable against the same x264 corpus + a JM-reference comparison | Differential fuzz against ffmpeg/JM is the canonical regression gate per M3 §3, runs the same way locally and on a GPU box. |
