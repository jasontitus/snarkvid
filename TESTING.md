...
| **`crates/h264-decoder`** | 117 unit tests across nine modules: `bitreader`, `nal`, `cavlc`, `transform`, `quant`, `slice`, `intra`, `mb`, **`frame`**. **`decode_iframe(bitstream) -> DecodedFrame` works end-to-end on the live x264 corpus**: walks the NAL stream, parses SPS+PPS+slice header, parses the MB header (mb_type, Intra_4×4 mode records, chroma intra mode, CBP, mb_qp_delta), returns a 16×16 YUV frame. Pixels are still placeholder (residual integration deferred — gated on filling in the rest of the CAVLC tables); the architectural pipeline is fully wired. Hadamard-DC quant variants for Intra_16×16 luma DC and chroma DC also landed (small and mechanical, completes the quant module). Crate is no_std-pure. | PASS | `cargo test -p snarkvid-h264-decoder` |
