...
| **`crates/h264-decoder`** | 108 unit tests across eight bottom-up modules: `bitreader`, `nal`, `cavlc`, `transform`, `quant`, `slice`, `intra`, `mb`. Highlights: live x264 corpus parse confirming SPS → PPS → IDR ordering; FDCT → quant → dequant → IDCT round-trip recovers the original block to within ±1; all 17 intra prediction modes; end-to-end `decode_residual_block_4x4` composition; **`mb.rs` parses a full macroblock header** (mb_type for the 26 I-slice variants per Table 7-11, 16 Intra_4×4 mode records with the explicit-rem one-skip rule, intra_chroma_pred_mode, CBP via the 48-entry Table 9-4 lookup, mb_qp_delta) — the entire MB header except residual data is now structured. Crate is no_std-pure. | PASS | `cargo test -p snarkvid-h264-decoder` |
...
| TBD | M3 corpus integration test: decode noise-16×16-qp18 corpus end-to-end and compare to ffmpeg-decoded reference YUV | Lands once `frame::decode_iframe` works. The corpus already ships the ffmpeg-decoded YUV alongside the H.264 bitstream — diff our decode output against it, expect bit-exactness on Y, U, V planes. This is M3 §3.1 acceptance ("Native parity"). |
...
| **M3 H.264 decoder modules still TODO** | Committed: `bitreader`, `nal`, `cavlc` (primitives + coeff_token VLC0 TC≤8 + chroma_dc + fixed_len_6 + level decode + total_zeros TC≤2 + run_before zl≤3 + decode_residual_block composition), `transform`, `quant`, `slice`, `intra` (all 17 modes), `mb` (mb_type / Intra_4×4 mode info / chroma intra mode / CBP / mb_qp_delta header parse). **Still TODO**: total_zeros tables for TC=3..15 (~250 entries); run_before tables for zeros_left=4..14 (~80 entries); coeff_token VLC0 rows TC=9..16, full VLC1 (nC ∈ [2,4)) and VLC2 (nC ∈ [4,8)) tables (~150 entries); VUI block parser inside SPS (currently skipped — works for our corpus); Hadamard-DC quant variants (Intra_16×16 luma DC and chroma DC); residual block reads inside `mb.rs` (16 luma 4×4, 4 chroma DC, 4 chroma AC); `frame.rs` top-level decode_iframe with neighbor-mode tracking; ffmpeg-reference parity test on the corpus. The remaining tables are mechanical transcription from spec Tables 9-7 / 9-9 / 9-10. | Differential fuzz against ffmpeg/JM (M3 §3) lands once `frame::decode_iframe` is functional; runs on CPU but takes wall-clock budget that suits a GPU box. |
