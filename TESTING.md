...
| **`crates/h264-decoder`** | 41 unit tests across four bottom-up modules: `bitreader` (raw bit reads MSB-first across byte boundaries; `read_ue`/`read_se`/`read_te` against H.264 spec Table 9-1 vectors; pathological-long-zeros rejection; truncation propagation), `nal` (3-byte + 4-byte Annex-B start codes; trailing-zero padding strip; `nal_unit_type`/`nal_ref_idc` extraction with `forbidden_zero_bit` rejection; emulation-prevention byte removal in all positions; **live integration test that parses the x264 corpus and confirms SPS → PPS → IDR ordering**), `cavlc` (the `read_vlc` table primitive; four `coeff_token` variants per Table 9-5: VLC0 nC<2 known codewords TC=0..8, chroma DC 4:2:0 known codewords, fixed-length-6 nC≥8 with the special-case 0b000011 sentinel, variant selector from nC), and `transform` (4×4 integer IDCT 1D butterfly per spec §8.5.12; DC-only-input → constant block; arithmetic shift behavior on `>>1` of odd values; round_shift_6 negative-rounding edge cases at -32 / -33; 4×4 Hadamard for Intra_16x16 luma DC and 2×2 Hadamard for chroma DC, both verified self-inverse up to 16× and 4× respectively). Crate is no_std-pure. | PASS | `cargo test -p snarkvid-h264-decoder` |
...
| **M3 H.264 decoder modules still TODO** | `bitreader`, `nal`, `cavlc` (primitives + coeff_token only), `transform` (IDCT + Hadamard) are committed and tested. **Still TODO**: full CAVLC tables (level prefix+suffix, total_zeros, run_before, VLC0 rows TC=9..16, full VLC1 / VLC2), `quant`, `slice`, `intra`, `mb`, `frame`. Each remaining module is bottom-up testable against the corpus. | Differential fuzz against ffmpeg/JM (M3 §3) lands once `frame::decode_iframe` is functional; runs on CPU but takes wall-clock budget that suits a GPU box. |
