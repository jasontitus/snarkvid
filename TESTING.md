...
| **`crates/h264-decoder`** | 56 unit tests across six bottom-up modules: `bitreader`, `nal`, `cavlc`, `transform`, `quant`, `slice`. Highlights: live x264 corpus parse confirming SPS → PPS → IDR ordering; `coeff_token` decoder against H.264 Table 9-5 known codewords; 4×4 IDCT 1D butterfly verified against spec §8.5.12 hand-computed inputs; `inverse_quant_4x4_ac` against spec §8.5.9 hand-computed values at qp=0/6/18/24/30 covering both the divisive (qp<24) and multiplicative (qp≥24) paths; **end-to-end FDCT → forward quant → inverse quant → IDCT → round_shift_6 round-trip recovers the original block to within ±1**; **slice.rs parses the live x264 corpus's SPS (16×16 dims, baseline profile), PPS (CAVLC, single slice group), and IDR slice header (slice_type I, SliceQPY=15 matching x264's actual rate-control choice)** with M3-scope validation that rejects CABAC, multiple slice groups, weighted prediction, B/P slices, etc. Crate is no_std-pure. | PASS | `cargo test -p snarkvid-h264-decoder` |
