...
| **`crates/h264-decoder`** | 71 unit tests across seven bottom-up modules: `bitreader`, `nal`, `cavlc`, `transform`, `quant`, `slice`, `intra`. Highlights: live x264 corpus parse confirming SPS → PPS → IDR ordering; `coeff_token` decoder against H.264 Table 9-5 known codewords; 4×4 IDCT 1D butterfly verified against spec §8.5.12 hand-computed inputs; `inverse_quant_4x4_ac` against spec §8.5.9 hand-computed values at qp=0/6/18/24/30 covering both divisive (qp<24) and multiplicative (qp≥24) paths; **end-to-end FDCT → forward quant → inverse quant → IDCT → round_shift_6 round-trip recovers the original block to within ±1**; **slice.rs parses the live x264 corpus** (16×16 dims, baseline profile, CAVLC, IDR slice with SliceQPY=15); **all 9 Intra_4×4 prediction modes** (Vertical / Horizontal / DC / DiagonalDownLeft / DiagonalDownRight / VerticalRight / HorizontalDown / VerticalLeft / HorizontalUp), each verified to produce a constant block from constant neighbors plus DC fallback rules (top-only / left-only / neither). Crate is no_std-pure. | PASS | `cargo test -p snarkvid-h264-decoder` |
...
| **M3 H.264 decoder modules still TODO** | `bitreader`, `nal`, `cavlc` (primitives + coeff_token only), `transform` (IDCT + Hadamard), `quant` (inverse_quant_4x4_ac path), `slice` (SPS/PPS/slice-header), `intra` (Intra_4×4 — 9 modes) are committed and tested. **Still TODO**: full CAVLC tables (level prefix+suffix, total_zeros, run_before, VLC0 rows TC=9..16, full VLC1 / VLC2); VUI block parser inside SPS (currently skipped — works for our corpus); Hadamard-DC quant variants for Intra_16x16 luma DC and chroma DC; `intra` Intra_16×16 (4 modes) and chroma 8×8 (4 modes); `mb`, `frame`. Each remaining module is bottom-up testable against the corpus. | Differential fuzz against ffmpeg/JM (M3 §3) lands once `frame::decode_iframe` is functional; runs on CPU but takes wall-clock budget that suits a GPU box. |
