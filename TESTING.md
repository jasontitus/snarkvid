...
| **`crates/h264-decoder`** | 51 unit tests across five bottom-up modules: `bitreader`, `nal`, `cavlc`, `transform`, `quant`. Highlights: live x264 corpus parse confirming SPS → PPS → IDR ordering; `coeff_token` decoder against H.264 Table 9-5 known codewords; 4×4 IDCT 1D butterfly verified against spec §8.5.12 hand-computed inputs; `inverse_quant_4x4_ac` against spec §8.5.9 hand-computed values at qp=0/6/18/24/30 covering both the divisive (qp<24) and multiplicative (qp≥24) paths; **end-to-end FDCT → forward quant → inverse quant → IDCT → round_shift_6 round-trip recovers the original block to within ±1**, validating that transform + quant compose correctly. Crate is no_std-pure. | PASS | `cargo test -p snarkvid-h264-decoder` |
...
| **M3 H.264 decoder modules still TODO** | `bitreader`, `nal`, `cavlc` (primitives + coeff_token only), `transform` (IDCT + Hadamard), `quant` (inverse_quant_4x4_ac path) are committed and tested. **Still TODO**: full CAVLC tables (level prefix+suffix, total_zeros, run_before, VLC0 rows TC=9..16, full VLC1 / VLC2), Hadamard-DC quant variants for Intra_16x16 luma DC and chroma DC, `slice`, `intra`, `mb`, `frame`. Each remaining module is bottom-up testable against the corpus. | Differential fuzz against ffmpeg/JM (M3 §3) lands once `frame::decode_iframe` is functional; runs on CPU but takes wall-clock budget that suits a GPU box. |
